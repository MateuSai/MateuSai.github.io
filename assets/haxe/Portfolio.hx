...
class Portfolio {
	...
	static function switchToGame(game: Game) {
		...
		if (activeGame == PLACEHOLDER) {
			Browser.document.getElementById("game-placeholder").remove();
		} else if (activeGame == GAME_0) {
			Browser.document.getElementById("game_0").remove();
		} else if (activeGame == GAME_1) {
			Browser.document.getElementById("game_1").remove();
		}
